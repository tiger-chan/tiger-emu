...
use generator::*;
pub use generator::{AddrModeData, OperData};
...
pub type Instruc = fn(&mut Registers, &mut dyn Bus) -> (AddrModeData, OperData);
...
mod test {
    ...
    #[test]
    fn nestest_log() -> Result<(), std::io::Error> {
        init();
        ...
        for instruction in logs {
            ...
            board.run_pc(next_pc);
            let (ppu, cpu) = board.instruction_state();

            let first_addr = |a| match a {
            ...
            let second_addr = |a| match a {
                AddrModeData::A | AddrModeData::Imp => {
                    format!("{:<28}", "")
                }
                AddrModeData::Abs(lo, hi) => {
                    format!("${:>04X}{:23}", lo as u16 | (hi as u16) << 8, "")
                }
            ...
            let last_instruction = format!("{:>04X}  {:>02X} {} {} {}A:{:>02X} X:{:>02X} Y:{:>02X} P:{:>02X} SP:{:>02X} PPU:{:>3},{:>3} CYC:{}",
                    cpu.reg.pc, cpu.opcode, first_addr(cpu.addr), cpu.op, second_addr(cpu.addr), cpu.reg.ac, cpu.reg.x, cpu.reg.y, u8::from(cpu.reg.p), cpu.reg.sp, ppu.1, ppu.0, cpu.tcc
            );
